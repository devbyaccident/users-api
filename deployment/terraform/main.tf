...
module "app" {
  source                          = "git@github.com:kids-first/aws-ecs-service-type-1.git"
  create_service_discovery        = var.create_service_discovery
  task_definition                 = var.task_definition == "" ? data.template_file.task_definition.rendered : var.task_definition
  region                          = var.region
